 ...
 *                         passed as printPipeContext's parameter. Optional.
 * @param {func} printRDDElement  Use this function to customize how to pipe elements. This function
 *                        will be called with each RDD element as the 1st parameter, and the
 *                        print line function (like out.println()) as the 2nd parameter. Optional.
 *                        An example of pipe the RDD data of groupBy() in a streaming way,
 *                        instead of constructing a huge String to concat all the elements:
 *                        def printRDDElement(record:(String, Seq[String]), f:String=&gt;Unit) =
 *                          for (e &lt;- record._2){f(e)}
 * @param {boolean} separateWorkingDir  Use separate working directories for each task. Optional.
 ...
 * }}}
 *
 * @param {number} num  k, the number of elements to return
 * @param {PARMNOTFOUND} ord  the implicit ordering for T
 ...
 * @returns {object}
 */
RDD.prototype.treeAggregate = function(zeroValue) {
    var zeroValue_uw = Utils.unwrapObject(zeroValue);
    var javaObject = this.getJavaObject().treeAggregate(zeroValue_uw);
 ...
 * @returns {object}
 */
RDD.prototype.treeReduce = function(f,depth) {
    var sv = Utils.createJavaParams(f);
    var fn = new org.eclairjs.nashorn.JSFunction(sv.funcStr, sv.scopeVars);
